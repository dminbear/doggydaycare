...
//////daniel’s viewService 
	...
	private void customersThatBoughtAllProducts() throws SQLException{
		...
		try{
			stmt = con.createStatement();
			rs = stmt.executeQuery("SELECT c.c_id,c.c_name FROM customer c WHERE NOT EXISTS ((SELECT p.p_id FROM products_t1 p) MINUS (SELECT b.p_id FROM buys b WHERE c.c_id=b.c_id))");
